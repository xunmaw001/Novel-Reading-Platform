...
public class ShujiLiuyanController {
    ...
    */
    ...
    public R update(@RequestBody ShujiLiuyanEntity shujiLiuyan, HttpServletRequest request){
...
//            shujiLiuyan.setYonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        //根据字段查询是否有相同数据
        Wrapper<ShujiLiuyanEntity> queryWrapper = new EntityWrapper<ShujiLiuyanEntity>()
            .eq("id",0)
            ;

        logger.info("sql语句:"+queryWrapper.getSqlSegment());
        ShujiLiuyanEntity shujiLiuyanEntity = shujiLiuyanService.selectOne(queryWrapper);
        shujiLiuyan.setUpdateTime(new Date());
        if(shujiLiuyanEntity==null){
            //  String role = String.valueOf(request.getSession().getAttribute("role"));
            //  if("".equals(role)){
            //      shujiLiuyan.set
            //  }
...
//                            shujiLiuyanEntity.setShujiId(Integer.valueOf(data.get(0)));   //小说 要改的
//                            shujiLiuyanEntity.setYonghuId(Integer.valueOf(data.get(0)));   //用户 要改的
//                            shujiLiuyanEntity.setShujiLiuyanText(data.get(0));                    //留言内容 要改的
//                            shujiLiuyanEntity.setReplyText(data.get(0));                    //回复内容 要改的
//                            shujiLiuyanEntity.setInsertTime(date);//时间
//                            shujiLiuyanEntity.setUpdateTime(new Date(data.get(0)));          //回复时间 要改的
