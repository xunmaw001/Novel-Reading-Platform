        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? '小说名称' : ''">
                        <el-input prefix-icon="el-icon-search" v-model="searchForm.shujiName" placeholder="小说名称" clearable></el-input>
                    </el-form-item>
                                         
                    <el-form-item :label="contents.inputTitle == 1 ? '小说类型' : ''">
                        <el-select v-model="searchForm.shujiTypes" placeholder="请选择小说类型">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('shujiLiuyan','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/xiaoshuoyuedupingtai/upload/shujiLiuyanMuBan.xls"
                        >批量导入小说留言数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('shujiLiuyan','导入导出')"
                                style="display: inline-block"
                                action="xiaoshuoyuedupingtai/file/upload"
                                :on-success="shujiLiuyanUploadSuccess"
                                :on-error="shujiLiuyanUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('shujiLiuyan','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "shujiLiuyan.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="shujiLiuyanText"
                                   header-align="center"
                                   label="留言内容">
                        <template slot-scope="scope">
                            {{scope.row.shujiLiuyanText}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                                     ...
                                     label="操作">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('shujiLiuyan','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('shujiLiuyan','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">回复</el-button>
                            <el-button v-if="isAuth('shujiLiuyan','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
    ...
    //当前表下拉框搜索条件
        ...
        methods: {
            ...
            getDataList() {
                ...
                params['shujiLiuyanDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "shujiLiuyan/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            shujiLiuyanUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "shujiLiuyan/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
